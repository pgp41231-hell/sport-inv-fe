...
function RequestCard({ request, user, sports, onChanged, notify }) {
  ...
  const finishQrDrag = (event) => {
    if (qrDragStart.current === null) return;
    if (event.clientY - qrDragStart.current < -55) setQrOpen(false);
    setQrDragOffset(0);
    qrDragStart.current = null;
  };
  return <article className={`panel equipment-request-card ${qrOpen ? "qr-expanded" : ""}`}>
    <div className="request-card-head"><div><strong>{label(request.requestType)} request</strong><small>{request.requesterName || request.requesterEmail}{request.teamName ? ` · ${request.teamName}` : ""}</small></div><span className={`status status-${String(request.status).toLowerCase()}`}>{label(request.status)}</span></div>
    <div className="request-items">{request.items?.map((item) => <span key={item.equipmentId}>{item.name} × {item.quantity}</span>)}</div>
    {request.dueAt && <p className={new Date(request.dueAt) < new Date() && request.status === "ISSUED" ? "overdue" : "muted-copy"}><Clock3 size={14} />Due {new Date(request.dueAt).toLocaleString("en-IN")}</p>}
    <div className="request-actions">
      {request.status === "PENDING" && canApprove && <><button className="button button-danger-soft" onClick={() => decide("reject")}><X size={15} />Reject</button><button className="button button-primary" onClick={() => decide("approve")}><Check size={15} />Approve</button></>}
      {request.status === "APPROVED" && request.requesterId === user.id && <button className="button button-primary" onClick={generate} aria-expanded={qrOpen}><QrCode size={16} />{qrOpen ? "Hide QR" : "Show QR"}</button>}
      {request.status === "ISSUED" && request.requesterId === user.id && request.requestType !== "RETURN" && <button className="button button-secondary" onClick={returnItems}><RotateCcw size={16} />Return items</button>}
    </div>
    {qr && <div className={`qr-reveal ${qrOpen ? "open" : ""}`} aria-hidden={!qrOpen}>
      <div className="qr-card" style={{ "--qr-drag-offset": `${qrDragOffset}px` }} onPointerDown={(event) => { qrDragStart.current = event.clientY; event.currentTarget.setPointerCapture?.(event.pointerId); }} onPointerMove={(event) => { if (qrDragStart.current !== null) setQrDragOffset(Math.min(0, event.clientY - qrDragStart.current)); }} onPointerUp={finishQrDrag} onPointerCancel={finishQrDrag}>
        <button className="qr-drag-handle" type="button" aria-label="Drag up or click to hide QR" onClick={() => setQrOpen(false)}><span /><small>Drag up to close</small></button>
        <div className="qr-content"><QRCodeSVG value={qr.token} size={210} level="M" /><div className="qr-details"><span className="status status-approved">Ready for collection</span><strong>{label(qr.purpose)} equipment QR</strong><p>Show this code at the inventory kiosk. It can be used once.</p><small>Expires {new Date(qr.expiresAt).toLocaleString("en-IN")}</small><code>{qr.token}</code></div></div>
      </div>
    </div>}
  </article>;
}
...
export default function EquipmentModule({ user, equipment, notify, onLogout }) {
  ...
  const openNewRequest = () => { setActiveView("request"); setForm((current) => ({ ...current, requestType: "CASUAL", equipmentId: "", quantity: 1 })); setShowForm(true); };
  ...
  const pendingCount = visibleRequests.filter((item) => item.status === "PENDING").length;
  const requestViewLabel = ["approver", "admin"].includes(user.role) ? "Requests & approvals" : "My requests";
  const selected = choices.find((item) => item.id === form.equipmentId);
  return <div className="page-stack">
    <header className="page-header"><div><p className="eyebrow">Equipment custody</p><h1>Equipment</h1><p>{activeView === "request" ? "Choose equipment and submit a request for collection." : "Track pending requests, approvals, QR codes, and issued items."}</p></div>{activeView === "request" && <button className="button button-primary" onClick={showForm ? () => setShowForm(false) : openNewRequest}>{showForm ? <X size={17} /> : <Plus size={17} />}{showForm ? "Close request" : "New request"}</button>}</header>
    <nav className="equipment-view-tabs" aria-label="Equipment sections">
      <button type="button" className={activeView === "request" ? "active" : ""} aria-current={activeView === "request" ? "page" : undefined} onClick={() => setActiveView("request")}><Plus size={17} /><span><strong>Request equipment</strong><small>Browse equipment and create a request</small></span></button>
      <button type="button" className={activeView === "activity" ? "active" : ""} aria-current={activeView === "activity" ? "page" : undefined} onClick={() => setActiveView("activity")}><Clock3 size={17} /><span><strong>{requestViewLabel}</strong><small>Pending, approved, and issued items</small></span>{pendingCount > 0 && <b>{pendingCount}</b>}</button>
    </nav>
    {activeView === "request" && <>
